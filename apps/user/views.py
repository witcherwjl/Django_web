...
class RegisterView(View):
    ...
    def post(self, request):
        ...
        # return HttpResponse('邮箱')
        if allow != 'on':
            return render(request, 'register.html', {'errmsg': '请同意协议'})
            # return HttpResponse('协议')
...
class ActiveView(View):
    def get(self, request, info):
        """用户认证"""
        ser_obj = Serializer(settings.SECRET_KEY,3600)

        try:
            raw_data = ser_obj.loads(info)
...
#     subject = '测试'
#     message = '123'
#     html_message = '<a href="http:http://127.0.0.1:8000/">首页</a>'
#     sender = settings.EMAIL_FROM
#     receiver = ['836691447@qq.com']
#     send_mail(subject, message, sender, receiver, html_message=html_message)
...
class UserInfoView(LoginRequiredMixin,View):
    ...
    def get(self, request):
        ...
        except Address.DoesNotExist as ret:
            address = None
